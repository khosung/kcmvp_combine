...
def _merge_adjacent_math(text: str) -> str:
    """Merge adjacent inline math blocks separated by a single space.

    Also removes 1-character suffix-prefix duplicates that arise from
    HyhwpEQ textline boundary overlaps in Korean PDF exports.
    For example: ``$Le$ $en(X)$`` → ``$Len(X)$`` (boundary 'e' deduplicated).
    """
    def _merge_pair(m: re.Match) -> str:
        a, b = m.group(1), m.group(2)
        # Deduplicate a 1-char overlap at the textline boundary.
        if a and b and a[-1] == b[0]:
            return f"${a}{b[1:]}$"
        # Deduplicate a LaTeX command overlap (e.g. \parallel\parallel boundary).
        cmd_match = re.search(r'(\\[a-zA-Z]+)$', a)
        if cmd_match and b.startswith(cmd_match.group(1)):
            cmd = cmd_match.group(1)
            return f"${a}{b[len(cmd):]}"
        return f"${a} {b}$"

    prev = None
    while prev != text:
        prev = text
        text = re.sub(r"\$([^$]*)\$ \$([^$]*)\$", _merge_pair, text)
    return text
            ...
            with TimeRecorder(conv_res, "table_structure"):
                ...
                for table_cluster, tbl_box in in_tables:
                    ...
                    for element in table_out["tf_responses"]:
                        ...
                        # Merge adjacent formula fragments from word tokenisation.
                        # TableFormer v1 stores matched tokens in text_cell_bboxes;
                        # pull them out, merge any adjacent $...$ blocks, and put the
                        # result in bbox.token so that TableCell picks it up correctly.
                        text_cell_bboxes = element.get("text_cell_bboxes")
                        if text_cell_bboxes:
                            raw = " ".join(
                                el.get("token", "") for el in text_cell_bboxes
                            ).strip()
                            merged = _merge_adjacent_math(raw)
                            if isinstance(element.get("bbox"), dict):
                                element["bbox"]["token"] = merged
                            element.pop("text_cell_bboxes", None)
                        elif isinstance(element.get("bbox"), dict) and isinstance(
                            element["bbox"].get("token"), str
                        ):
                            element["bbox"]["token"] = _merge_adjacent_math(
                                element["bbox"]["token"]
                            )
