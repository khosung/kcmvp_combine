...
def _infer_heading_level(text: str) -> int:
    """Infer a heading level (1–5) from the heading text.

    The docling markdown export renders SectionHeaderItem with ``level+1`` hashes
    (level=1 → ##, level=2 → ###, etc.).  We return values 1–5 so the output
    matches typical Korean government/notice document nesting:

    level 1 → ## : root section title (pure title, no numbering)
    level 2 → ### : numbered sections "N ..."
    level 3 → #### : sub-sections "가. / 나." or circled digits "①②"
    level 4 → ##### : lowest level "■ / ◆" bullet headings
    """
    t = text.strip()
    # Level 4 → ##### : special bullet markers
    if re.match(r'^[■◆◇▪▫]', t):
        return 4
    # Level 3 → #### : Korean syllable ordinal "가." etc. or circled digits ①②…
    if re.match(r'^[(（]?[가나다라마바사아자차카타파하][)）.．]', t) or re.match(r'^[①②③④⑤⑥⑦⑧⑨⑩]', t):
        return 3
    # Level 2 → ### : starts with digit + space (numbered section)
    if re.match(r'^\d+[\s\.]', t):
        return 2
    # Default level 1 → ##
    return 1
    ...
    def _add_child_elements(
        ...
        for child in element.cluster.children:
            ...
            if c_label == DocItemLabel.LIST_ITEM:
                ...
                doc.add_heading(
                    parent=doc_item, text=c_text, prov=c_prov,
                    level=_infer_heading_level(c_text)
                )
            else:
    ...
    def _handle_text_element(self, element, out_doc, current_list, page_height):
        ...
        if label == DocItemLabel.LIST_ITEM:
            ...
            new_item = out_doc.add_heading(
                text=cap_text, prov=prov, level=_infer_heading_level(cap_text)
            )
